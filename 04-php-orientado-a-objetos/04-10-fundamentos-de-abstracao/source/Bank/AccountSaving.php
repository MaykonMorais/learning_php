...
class AccountSaving extends Account
{
    ...
    public function withdrawal($value)
    {
        if($this->balance >= $value) {
            ...
            Trigger::show("Saque de R$ {$this->toBrl($value)} realizado com sucesso", Trigger::ERROR);
        } else {
            Trigger::show("Saldo insuficiente, você tem R$ {$this->toBrl($this->balance)}!", Trigger::WARNING);
        }
    }
